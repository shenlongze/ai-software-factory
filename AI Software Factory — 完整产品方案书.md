...
> 2026-08-22 补充（用户指出）: 系统产生大量数据（执行/资产/治理/画像/成本），
> **可视化此前未系统设计**——本节补"呈现什么 + 怎么呈现 + 数据从哪来"。

#### 5.7.1 可视化分层

| 层 | 形态 | 适用 | 状态 |
|---|---|---|---|
| CLI 文本视图 | 表格/树/摘要（exec history/project status/evidence show） | 终端/自动化 | ✅ 已实现 |
| **Web 仪表盘** | 图表/看板/实时 | 人类浏览/管理 | 📐 M5/M7 |
| 实时监控视图 | 指标流/告警 | 运维 | 📐 M5 |

#### 5.7.2 要可视化的数据（都来自已有数据源）

```
执行/项目: 项目状态 · 执行时间线 · 任务 DAG · 进度 · 失败原因
资产/证据: 证据包（diff/测试/决策）· 资产版本链 · 产物视图
治理:      审批队列 · 风险分级 · 审计链（血缘图）· 合规报告
Agent:     团队视图（角色/技能/负载）· 画像（成功率/成本/速度）· 协作图
成本/资源: 预算水位 · 每任务成本 · 单位经济
监控:      实时指标 · 告警
```
...
| 视图 | 内容 | 数据源 |
|---|---|---|
| 项目看板 | 项目列表 + 状态/PRD/管线/证据 | §1.4 + observability |
| 执行时间线 | 事件流（谁/何时/工具/模型/成本） | execution_timeline |
| 任务 DAG | 任务树 + 关键路径 + 并行 | execution_state + §3.9 |
| 证据查看器 | 证据包（diff/测试/决策链） | evidence.py |
| 审批中心 | 待审批队列 + 分级 + 决策 | approval/review_gate |
| 审计探索器 | 事件链 + 血缘图 + 过滤 | audit |
| 团队视图 | Agent 角色/负载/画像 | agents + metrics |
| 成本面板 | 预算水位 + 每任务成本 + ROI | budget + usage |
| 监控告警 | 实时指标 + 告警列表 | observability 实时（M5） |

#### 5.7.4 设计原则

- **数据已就绪**：所有视图数据来自已有模块（统一契约 §2.10-11），可视化只是"读 + 渲染"，不造新数据源
- **CLI 先行**：CLI 文本视图已实现（✅）——Web 仪表盘是同一数据的另一渲染层
- **只读**：可视化只读，操作仍走 API/CLI（§2.10 只读/写分离）
- **权限**：视图按角色（§6.3 RBAC 落地后）——审计视图限 Auditor
...
|---|---|
| CLI 文本视图（9 类数据） | ✅ 已实现 |
| Web 仪表盘（视图清单） | 📐 M5（Web 入口打通）/ M7（IDE 内联） |
| 实时监控可视化 | 📐 M5 |
