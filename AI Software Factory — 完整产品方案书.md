...
#### 8.5.8 历史数据迁移与回填（档1 → 档2/3/4 的存量数据怎么办）★

> 2026-08-22 补充（用户关键问题）: 先档1 用一段时间，再上 DB/RAG——**档1 的存量数据如何同步到新存储**。
> 原则: 全量回填 + 增量双写 + 可回滚 + 尽量零停机。

**档1 有哪些历史数据（全部本地 JSON，统一契约 §2.10 → 迁移容易）**

```
projects/<slug>/（product.json / PRD / artifacts / evidence / execution_state）
exec/execution_records.json + results + agent_metrics.json
memory/（experience 经验库）
audit/audit_events.json（审计链）
agents/agents.json（Agent 画像）
```
...
```
① 全量回填（backfill，一次性）:
   结构化（项目/经验/审计/执行记录）→ 导入 DB（按统一 id，幂等去重）
   语义（文档/经验文本）→ 向量化（embedding）→ 生成索引
② 增量双写（切换后）:
   新数据同时写 旧文件 + 新存储（或经事件总线同步，§2.12）
③ 切换:
   读取从"双读（新旧都查）"过渡到"只读新存储"
   → 迁移失败可回滚（旧文件保留）
```
...
| 项 | 说明 |
|---|---|
| 幂等 | 迁移按统一 id 去重，重复运行安全 |
| 可回滚 | 迁移期间旧文件不删；失败 → 回退档1 |
| 零停机 | 先建新库 + 后台迁移 → 切换读取 → 双写（不中断服务） |
| 向量回填 | 非结构化经验/文档一次性 embedding（档3 才需要） |
| 命令 | `factory migrate --target db|vector`（升级时运行；M5 落地） |

**与 §8.5.4 的关系**：§8.5.4 讲"调用代码无痛"；本节讲"**存量数据无痛**"——两者合起来才是完整无痛升级。
