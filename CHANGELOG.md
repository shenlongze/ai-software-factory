...
### Added
- 会话里直接说「**退出工作目录** / **回到全局** / **退出项目**」⇒ 回到全局数据 ✓
- ★ 正则**要求带对象** ✗：光说「退出」「返回」**不**匹配（那可能是想退出程序 ✗ 不抢它的语义 ✓）

### 实测（pty 真跑 ✓）
```
factory> 进入飞机大战项目
系统 ▸ 已进入工作目录: plane-shooter（P-6eea9b3e）（之后只读命令默认只跑它; /project 退出 回全局 ✓）
factory> 退出工作目录
系统 ▸ 已退出工作目录（plane-shooter（P-6eea9b3e））—— 回到全局数据 ✓
```

### 守卫
- 「会话直接说进入」加两条：**必须有自然语言退出** ✗（防半成品）+ 退出正则**必须带对象** ✗
  （防误抢"退程序"的语义 ✓）

## [v1.3.45] — 2026-09-24

**会话里直接说「进入飞机大战项目」就能进**（Founder:「不实用命令，直接使用会话，是否可以进入」）✓。

### 实测原状（✗ 两层问题）
1. 说「进入飞机大战项目」⇒ 会话先 `project show 飞机大战`（**匹配不上** ✗ 项目名是 plane-shooter）
   ⇒ 再 `project list`（**能靠说明找到** ✓）⇒ **但不会进入** ✗（它压根没有"进入"这个动作）
2. 加了提示词规则后 ⇒ 模型**口头说**「已进入 plane-shooter（P-6eea9b3e）项目 ✓」
   **却一个动作都没发** ✗✗ = **假声称**（比不会更坏: 老板以为进去了, 实际没有 ✓）

### Fixed
- **① 解析器认说明（需求原话）** ✓：`resolve_project_id` 增加第 ⑤ 条匹配（优先级最低, 不抢 id/名字 ✓）
  · 实测：「飞机大战」⇒ `P-6eea9b3e` ✓ ·「寄养」⇒ `P-9816c063` ✓ ·「不存在」⇒ 空（如实 ✗ 不编）
- **② 新增模型协议动作 `SESSION: project <名字|id|片段>`** ✓：会话可**替老板进入工作目录**
  （不是数据写入 ⇒ 不需要点头 ✓；从回答里清掉, 不露给老板 ✓）
- **③ ★ 确定性兜底（不靠模型自觉 ✗）**：welcome 里加正则 —— 老板这轮说了
  「进入/切到/切换到/只看 + 某个能**真解析**出来的项目」⇒ **CLI 直接进入** ✓
  · 抓到的词会剥掉尾巴的「项目/工作目录」（"进入飞机大战项目" ⇒ 抓成"飞机大战项目" ✗ ⇒ 修 ✓）
  · 解析不出来就不动 ✓（不猜 ✗）
- **④ 提示词新增规则 1f**：说「进入/切到」⇒ 用 `SESSION: project …`；**不许**让老板自己敲 `/project` ✗

### 实测（pty 真跑 ✓）
`进入飞机大战项目` ⇒ `系统 ▸ 已进入工作目录: plane-shooter（P-6eea9b3e）` ✓（真进去了 ✓）

### 守卫
- 新增「会话直接说进入」：说明能解析（行为断言 ✓）+ 确定性兜底必须在（**专门防"只靠模型"退化成假声称** ✗）+
  `SESSION:` 协议 + 提示词规则 ✓

## [v1.3.44] — 2026-09-24

**概念梳理（Founder: 「公司/部门/项目/工作空间，一定要梳理清晰」）** ⇒ 新增
`docs/概念梳理-组织与工作.md`（挂在 `AGENTS.md` 的「进入即读」入口 ✓）。

### 结论（全部**只读原文**得出, 无自造 ✓）
- **组织树 × 工作树是两套正交维度**，不是严格父子
  （`原文` 方案书 L235-238, 2026-08-24 Founder 澄清）
  ```
  组织树（归属谁）: 集团 → 公司 → 部门（可选）
  工作树（做什么）: 项目 → 任务 → 叶 → 执行
  关联（谁负责）  : Project.company_id · Project.department_ids[]（多对多 ✓）
  ```
- **「工作空间」不在这两棵树里** ✗ —— 它是**项目之上**的一层分组 + 跨项目运营视图
  （`原文` ADR-0017「workspace 层」· `--workspace` 自述「跨项目运营视图组」）
- **「工作目录」也不在这两棵树里** ✗ —— 它是**会话里选中的某一个项目**（v1.3.43 新做）
- **部门是可选的**（Solo 扁平 ⇒ 零配置；后期建公司再"关联"项目, 数据不动 ✓）

### 三条最容易混的边界（已写进文档 ✓）
1. 工作空间（多项目容器）≠ 工作目录（会话选中的某个项目）
2. 组织树 ≠ 工作树的父子关系 ⇒ 项目**不是**部门的子节点（要归属就"加关联"✓）
3. 部门可选 ⇒ 前期直接建项目 ✓

### 实现核对（读代码 ✓）
- `Project.department_ids` 是**列表** ✓ ⇒ 与方案书"多对多"一致 ✓
- `Company.parent_company` **已预留**（集团, Phase 21+ ✓）
- `factory create company|department|project` ✓ 三件都能建 ✓
- 缺口记入文档第四节（workspace.yaml 未建 ✗ · 旧会话无项目归属 ✗ · E26/E27 待补 ✗）

### 守卫
- 新增「概念梳理文档」：文档必须存在 + 含六个关键概念 + **标了原文出处**（禁凭记忆 ✗）+
  **挂进 AGENTS.md**（否则"梳理清晰"只停在聊天里, 进不了进入即读 ✗）

## [v1.3.43] — 2026-09-24

**工作目录**（Founder 设计：「会话内也可以选择项目，然后进入项目…相当于后面的会话只针对当前项目，
可以退出，页面的体现就是选择工作目录」✓）。

### 现状（先说已有的 ✓ 再说缺的 ✗）
- 已有：`/project <名字|id|片段>` 进入 ✓ · `/project 清空` 退出 ✓ · 状态栏常显 ✓ · 进提示词 ✓
- 缺：① 进入后**命令不会自动按项目跑** ✗ ② 没有「工作目录」说法 ✗ ③ 选择**没有编号** ✗
  ④ 会话没跟所选项目绑定 ✗ ⑤ 重开就丢 ✗

### Added（本轮做完 ①②③④）
- `/project` 列候选时**带编号**（1) 2) 3) … ✓）⇒ `/project 2` 直接进入 ✓（也可继续用名字/id/片段 ✓）
- 措辞统一改成 **工作目录**：`当前工作目录: …` / `已进入工作目录: …` / `/project 退出` ✓
- ★ **进入后只读命令默认只跑该项目**：执行前自动补 `--project <id>`；
  命令**不认**这个参数 ⇒ **自动回退**重跑原命令 ✓（不硬塞 ✗）
  · 抽共用助手 `workdir_project_id` / `argv_candidates`（**两处**命令执行点都用 ✓ 不复制粘贴 ✗）
  · 实测（pty）：`/project 2` ⇒ 问"状态总览" ⇒ 输出 **项目 1 · P-6eea9b3e** ✓（真收窄 ✓）
- 新建终端会话带上当前工作目录的 `project_id`（乙 的延续 ✓）

### 未做/已知问题（如实记）
- ⑤ **重开窗口不记忆**（未持久化 ✗）⇒ 记 TODO E26
- 显示小瑕：工具行仍显示模型原本的命令（未把补上的 `--project` 显示出来 ✗）⇒ 记 TODO E27
  （功能已生效 ✓ 只是**展示**不够透明 ✓）

### 守卫
- 新增「工作目录」：助手行为（取 id / 候选序列 / 无工作目录不加参）+ **两处执行点都必须用上助手**
  （我第一轮只接了一处 ⇒ 命令行不生效 ✗，这条守卫专门防它再次发生 ✓）+ 文案分支必须齐 ✓
...
### Fixed
- **language 不再是空的** ✗：库里存的是**创建时**的值（可能 `unknown`）⇒
  `project show` 现在对仓库**实时识别**语言/框架并显示（标「（自动识别）」）
  · 复用现成 `detect_language/detect_framework`（不重造轮子 ✓）
  · 只读命令**不写库**（ADR-0013 ✓）⇒ 只补显示 ✓
  · 实测：plane-shooter ⇒ `language javascript（自动识别）` ✓（此前 unknown ✗）
- **详情重排成三段**（此前是"字段+树+字段"混着排 ✗）：
  `── 基本信息 ──`（language / framework / repository）→
  `── 任务树与进度 ──`（共 N 棵 · 叶/完成/百分比 · 每棵明细 · 下一步）→
  `── 团队与能力 ──`（agents / skills / workflows）
  · 顺手删掉重复输出的旧尾巴（同一批数字打两遍 ✗）

### 会话提示词
- 新增规则 **1d**：被问「你能不能改代码」时，**要答平台整体** ——
  这层只跑只读查询 + 提交写命令（点头才跑）✓，但**平台会真正写代码**（走 `run`/`chain` 派活，
  agent 在项目仓库里改并自己提交 ✓）⇒ 不许说成「系统不会写代码」✗
  · 起因：Founder 实测里会话答「我改不了代码」✗ ⇒ 用户会以为整个平台不能写代码（而它刚写完 30 个叶 ✗）

### 守卫
- 「项目详情能看树与进度」加严：**三段标题必须齐** + 有代码的项目 `language` 不许是 unknown ✓
