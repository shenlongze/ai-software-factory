...
**⑤ 组织层级隔离（公司 / 部门 / 行业 × 资源 — §11.6 命令域配套）★**

> 2026-08-24 补充（Founder: 资源是否隔离? 烂事太多）: 项目隔离之上还有组织层级
> （公司→部门→行业工厂）。资源隔离 = **组织层级 × 资源可见性**, 规则必须统一。

**组织层级 × 资源可见性矩阵**

| 资源 | 隔离级别 | 规则 | 字段 |
|---|---|---|---|
| 公司级（agent/项目/知识/审计/经验/预算） | **公司隔离** | A 公司看不见 B 公司 | `company_id` |
| 部门级（项目/员工） | 部门可选 | 部门间可选隔离或公司内共享 | `department_id`（可空=公司级） |
| 行业级（skill/workflow/knowledge/template） | 行业隔离 | 行业工厂专属（FactorySpec） | `factory_id` |
| 全局（系统配置/内置 Skill） | 共享 | 所有组织可见 | — |
...
factory project list --company C-1 --department D-1   # 资源命令带组织作用域
资源命令缺省作用域 = 当前上下文（会话 current_company/current_department）
显式 --company/--factory 覆盖
```

**隔离实现三要素**

```
1. 字段: company_id / department_id / factory_id（资源模型统一加, 默认值向后兼容）
2. 查询过滤: 任何 list/show 按当前作用域过滤（§7.2.2 守则 1 扩展: project_id → 组织 id）
3. 审计: 组织 id 进审计事件（谁/哪个公司/哪个部门 操作了什么）
```

**与现有机制的衔接**

- §1.4.5 层级模型: 组织树（公司/部门）正交 工作树（项目/任务）
- §7.2.1 经验共享: 公司级经验共享仍过标签（通用全共享/同栈共享/项目特定）
  —— 组织隔离管"可见性", 标签管"复用边界", 两者叠加
- §11.6 命令域: 组织域命令带 --company/--factory 作用域

**结论**: 资源隔离 = **组织层级 × 资源可见性矩阵 + 命令作用域 + 查询过滤**。
公司隔离（硬）/部门可选（软）/行业隔离（FactorySpec）/全局共享（系统）——
四档规则统一, 组织命令和资源命令都有据可依, 不逐个重新想。
```
...
| **闭环接线**（经验→画像→决策引用→回写） | 设计（M4 自我提升闭环） | 🟡 部分（经验→画像→决策引用已通; 回写待做） |
| 可信度护栏 / 学习开关 | 设计（M4 C5） | 📐 |
...
| M4 自我提升闭环 | v1.3.0 | 经验→画像→决策引用 + 评价回写 + 护栏 | 🟡 部分已落（v1.3.0: 经验自动积累+按任务评估/推荐已通; 评价回写/护栏 待做） |
