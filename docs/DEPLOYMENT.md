...
> 版本: 1.1.0 | 目标: macOS/Linux 本地部署 (wheel 分发)
...
.factory-venv/bin/pip install dist/ai_software_factory-1.1.0-py3-none-any.whl

# 3. 验证
.factory-venv/bin/factory --version                # → AI Factory v1.1.0
```
...
```bash
curl http://127.0.0.1:8011/health     # {"status":"ok","version":"1.1.0"}
curl http://127.0.0.1:8011/ready      # {"status":"ready","data_dir":"...","issues":[]}
curl http://127.0.0.1:8011/version    # {"name":"ai-software-factory","version":"1.1.0"}
```
...
| 症状 | 原因 | 解决 |
|---|---|---|
| `No module named 'factory_console.audit'` | 旧 wheel 缺子包 | 重装 ≥1.1.0 wheel |
| start 找不到 python | 硬编码 .venv (已修复) | 升级到 ≥1.1.0 |
