...
> 版本: 1.1.9 | 目标: macOS/Linux 本地部署 (wheel 分发)
...
# 3. 验证
.factory-venv/bin/factory --version                # → AI Factory v1.1.9
```
...
```bash
curl http://127.0.0.1:8011/health     # {"status":"ok","version":"1.1.9"}
curl http://127.0.0.1:8011/ready      # {"status":"ready","data_dir":"...","issues":[]}
curl http://127.0.0.1:8011/version    # {"name":"ai-software-factory","version":"1.1.9"}
```
