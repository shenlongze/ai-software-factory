...
>
> 版本: **v1.1.334** · 更新: 2026-08-28 · 依据: 实测命令 + 代码核对 + CHANGELOG
...
### 7.7 会话 Agent 循环（原生 function calling + 计划→审批→执行）
- **说明**: 项目级会话默认走 Agent 循环 — **IntentCore 意图门第一步**（LLM 结构化理解 intent×target×need×emotion，不靠关键词）→ 按意图注入路由约束 → 模型原生 tool_calls 选工具（14 个会话动作工具）→ 执行 → 回喂 → 循环 → 最终回答（带证据）
- **意图路由**: 查询→数据工具带证据；质疑→强制自查重查→承认/修正（注入上一轮回答）；聊天→自然对话；开发/派活→出计划审批；操作→动作工具；外部→external_route；意图不明→直接追问（不进工具循环）
- **外部能力进工具面 (v1.1.209)**: 已导入外部 agent (codex.*/claude.*) 自动生成 delegate_external 工具 — 会话可直接委派真实执行 (统一记录/审计)；新增外部 agent 无需改代码
- **质疑自查加深 (v1.1.209)**: challenge 首轮仅验证工具 (不给动作/计划/外部)；未调验证工具直接答 → 强制驳回再查
- **上下文连贯性 (v1.1.210)**: Agent 主循环注入最近 4 轮历史 + 锚定任务上下文；WebUI 旧路由也注入历史（此前仅 CLI 有）
- **会话话题账本 (v1.1.211)**: 单会话多议题分块（延续判断+显式切换+回A切回）；注入=当前话题详细+其他话题一行摘要；同话题过长→滚动压缩（running summary）
- **意图解析 v2 (v1.1.215)**: LLM 语义优先, 关键词只做兜底/防退化；强分析信号分级；真实会话回归测试固化
- **AgentLoop v3 (v1.1.216)**: agentic 自主循环 (意图软参考, 不硬路由) + Reflection 每轮自评主动收敛；硬收敛保留
- **S10-126 M1 (v1.1.217)**: 对话自然度(风格分级/模板解放/情绪回应/详略) + 会话审计可观测 + 执行状态机(计划→逐任务委派+验证→交付)
- **S10-126 M2 (v1.1.218)**: 回答验证闭环(数字可复核/无证据不结论/自检) + 会话质量 eval(12 条数据集+LLM-judge+≥90% 门)
- **S10-126 M3 (v1.1.219)**: 项目级跨会话记忆(话题摘要→记忆→新会话注入) + knowledge_search 项目知识检索
- **read_code (v1.1.221)**: 读仓库代码文件(行号/分页/关键词定位/越界安全), 会话可讲代码逻辑
- **所答非所问治理 (v1.1.223)**: Reflection 回答前相关性自检 + 用户纠正(不是/我要的是)强制重对齐 + 硬收敛注入问题锚点
- **执行过程可视化 (v1.1.224)**: 会话消息展示工具调用徽章(✅/❌ 调用了 xx)；代码优先偏好(要架构/逻辑/源码→读代码不读文档)
- **计划→审批→执行**: 开发类需求 → plan_development 出计划（目标/任务/顺序/验收）→ 用户语义判断审批（可以/开始→execute_plan 真实建任务进 backlog；要改→重写计划；意图不明→追问）— 不靠关键词
- **真实代码扫描**: "扫描代码" → code_scan 确定性读盘（文件数/LOC/语言/测试/TODO/大文件/最近改动/git），不再答"未查询到代码扫描结果"
- **硬收敛护栏**: 工具调用达上限（MAX_TOOL_CALLS=6）→ 硬停，最后强制一轮收敛（不再给工具）；信息不足 → 明确追问（3 次 loop 后还不清醒就追问），不无限调研/不编造
- **入口**: WebUI 项目级会话（scope=project）自动；Agent 不可用 → 回退旧意图路由
- **状态**: ✅ · **起始**: v1.1.207
...
| 版本 | 核心功能 | 里程碑 |
|---|---|---|
| v1.1.224 | 执行过程可视化(工具调用徽章) + 代码优先偏好(要代码不读文档) | 后端/API/前端 |
| v1.1.223 | 所答非所问治理 (回答前相关性自检 + 用户纠正重对齐) | 后端/API |
| v1.1.222 | read_code 支持目录 (返回文件清单, 模型选文件再读) | 后端/API |
| v1.1.221 | read_code 工具 (读代码讲逻辑: 带行号/分页/关键词定位/越界安全) | 后端/API |
| v1.1.220 | 旧路由质疑信号 (是真正影响项目的么→deep_analyze, 无 LLM 也验证) | 后端/API |
| v1.1.219 | S10-126 M3: 跨会话记忆(项目级) + 项目知识检索接会话 | 后端/API |
| v1.1.218 | S10-126 M2: 验证闭环(数字可复核/无证据不结论/自检) + 会话 eval(12 数据集+judge) | 后端/API |
| v1.1.217 | S10-126 M1: 说人话(S-6 风格/模板解放) + 可观测(S-1 审计) + 做人事(S-5 执行状态机) | 后端/API |
| v1.1.216 | AgentLoop v3 (agentic 自主 + Reflection 自评, 意图门降级软参考) | 后端/API |
| v1.1.215 | 意图解析重构 (LLM 语义优先, 关键词只兜底/防退化) | 后端/API |
| v1.1.214 | 项目结构能力 (目录树/模块分布, '了解项目真实结构'不再答进度) | 后端/API |
| v1.1.213 | 扫描代码路由修复 (code_scan 真实读盘, 不再答'未执行代码扫描') | 后端/API |
| v1.1.212 | 怀疑式质疑识别 (是真正影响项目的么/靠谱吗 → challenge 验证) | 后端/API |
| v1.1.211 | 会话话题账本 (分块/取舍/压缩 — 聊B不带A细节, 回A切回) | 后端/API |
| v1.1.210 | 会话上下文连贯性 (Agent 主循环+WebUI 旧路由注入历史, 锚定任务上下文) | 后端/API |
| v1.1.209 | 外部能力进工具面 (delegate_external 动态委派) + 质疑自查加深 (验证前置) | 后端/API |
| v1.1.208 | IntentCore 意图理解层 (LLM 结构化意图 + 路由专业能力 + 质疑自查 + 意图不明追问) | 后端/API |
| v1.1.207 | 会话 Agent 循环 (原生 FC + 计划→审批→执行 + 硬收敛护栏 + 真实代码扫描) | 后端/API |
| v1.1.206 | 意图语义分析门 (分析评估措辞→deep_analyze, 不被命令词劫持) | 后端 |
| v1.1.205 | 会话分析不盲猜 (分析利弊→deep_analyze 多工具; 扫描加系列级深度) | 后端 |
| v1.1.204 | WebUI 会话事实卡进度用真实任务统计 (不再虚报 0%) | 后端 |
| v1.1.203 | CLI 会话上下文连续 (ChatService 多轮历史注入 prompt) | 后端/CLI |
| v1.1.202 | M7.2 审查验证钩子 (审查类任务 → reviewer 交叉审查 → PASS/FAIL 回写) | 后端/CLI |
| v1.1.201 | M7 验证钩子自动化 (auto 后自动 pytest/verify_hook → 效果分回写 → 路由学习) | 后端/CLI/WebUI |
| v1.1.200 | M6 WebUI 路由测试 + 路由接入执行链 (auto 全自动闭环) | WebUI/后端/CLI |
| v1.1.199 | M5 路由层 (任务分类→能力匹配→历史加权4/3/2/1→成本分级→用户显式→兜底) | 后端/CLI |
| v1.1.198 | 监控补维度 (成本可记录/时间粒度/内部vs外部对比/执行器对比) | WebUI/后端 |
| v1.1.197 | fix 监控菜单双 📊 (i18n label 去 emoji) | WebUI |
| v1.1.196 | i18n 补监控菜单名 (nav.workspace.monitor 中英) | WebUI |
| v1.1.195 | 📊 监控中心增强 (趋势/多维/记录流钻取/回修·验证分布/作用域) | WebUI/后端 |
...
| v1.1.186 | T-7 双轨对齐 (backlog 补勾 W-3/D-1/F-10, 计划与执行一致) | 治理 |
