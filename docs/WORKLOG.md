...
| v1.3.42 | **丙（一半）: status --project**（Founder「全做」第 3 件）: 按项目真收窄 —— 实测 全局 项目4/树10 → `--project plane-shooter` ⇒ 项目**1**/树**1** ✓; 复用 `progress.summary(project_id=…)` ✓; 根因修: `main.py` 里 `cmd_status(ctx)` **没传 args** ✗ ⇒ 参数型行为永远不生效 ⇒ 改 `cmd_status(ctx, args)` ✓; **未做**: `dashboard --project`（收集器已支持 project_id ✓ 但顶层 parser 没定位到 + 未接参）⇒ **撤掉空转 flag** ✗ 不留半成品 ⇒ 记 E24 ✓ | 守卫 70→71 |
| v1.3.43 | **工作目录**（Founder 设计: 会话里选项目/进入/之后只针对它/可退出, 页面体现=选工作目录 ✓）: 已有 /project 进入+清空退出 ✓ 缺 ①命令不自动按项目跑 ②无「工作目录」说法 ③选择无编号 ④会话未绑定; 本轮做齐 ①②③④: 候选带编号（/project 2 ✓）· 措辞改「工作目录」· **进入后只读命令自动补 --project, 不认则回退**（抽助手 workdir_project_id/argv_candidates, **两处**执行点都用 ✓）· 新会话带 project_id; 实测 pty: /project 2 ⇒ 「状态总览」⇒ 输出 项目**1** · P-6eea9b3e ✓ 真收窄 ✓; 未做: ⑤重开不记忆（E26）· 显示小瑕（工具行仍显原命令, E27）| 守卫 71→72 |
| v1.3.44 | **概念梳理**（Founder: 「公司/部门/项目/工作空间, 一定要梳理清晰」）: 新增 `docs/概念梳理-组织与工作.md`（**全按原文** ✓ 方案书 L235-238 Founder 2026-08-24 澄清 + ADR-0017）—— 核心结论: **组织树 × 工作树是两套正交维度**（项目**不是**部门子节点 ✗）; 工作空间=**多项目容器**+跨项目视图; 工作目录=**会话选中某个项目**（两者不同层 ✗）; 部门**可选**（Solo 零配置 ✓）; 核对实现: department_ids 是列表 ✓ 与原文「多对多」一致 · parent_company 已预留 ✓ · create company|department|project 都能建 ✓; 文档挂进 AGENTS.md「进入即读」+ 守卫（存在/含概念/标原文/挂入口）✓ | 守卫 72→73 |
| v1.3.45 | **会话直接说「进入X」就能进**（Founder:「不用命令, 直接用会话, 是否可以进入」）: 实测原状 = ① 说「进入飞机大战项目」只找到不会进（项目名 plane-shooter, 「飞机大战」只在说明里 ✗）② 加规则后模型**假声称**「已进入 ✓」却一个动作不发 ✗✗; 修 = ① `resolve_project_id` 第⑤条**认说明（需求原话）**（实测 飞机大战→P-6eea9b3e ✓ 寄养→P-9816c063 ✓）② 新协议 `SESSION: project …` ③ ★ **确定性兜底**（welcome 正则: 说「进入/切到/只看 + 能解析的项目」⇒ CLI 直接进, **不靠模型自觉** ✗ 防假声称）④ 提示词 1f; 实测 pty: 「进入飞机大战项目」⇒ 已进入工作目录 plane-shooter（P-6eea9b3e）✓ | 守卫 73→74 |
| v1.3.46 | **自然语言退出工作目录**（补 v1.3.45 的尾巴: 只能进不能出 = 半成品 ✗）: 说「退出工作目录/回到全局/退出项目」⇒ 回全局 ✓; 正则**要求带对象** ⇒ 光说「退出/返回」不抢「退程序」语义 ✗; 实测 pty: 进 plane-shooter ⇒ 「退出工作目录」⇒ 已退出 ✓ | 守卫 74（加严两条）|
| v1.3.47 | **回收卡死执行（E13）**（飞机大战被它卡住: run 整批只推几个叶, 其余全「推迟(容量受限)」✗）: 现场 = **4 个执行挂 RUNNING 5 天**（pid=None 无从核）⇒ 占并发容量; sweep_stale_claims(治认领)/recover(治树) 都不碰执行（注释自述「不会碰它」✓）; 修 = 新增 `sweep_stale_executions`（只按时间判死·阈值 60 分钟·**只标 FAILED 绝不标 SUCCESS** ✗·原因写 `_sweep_reason`）+ 接进 pump 清理点; 实测回收 EXR-011/021/022/025 ⇒ **RUNNING 0** ✓; 坑: 我第一版 import 了**不存在的模块** ⇒ 被 except 吞 ⇒ 静默空转 ✗（一个没回收）⇒ 用正确 API（同文件 32 行）改对 ✓ | 守卫 74→75 |
| v1.3.48 | **能力地图**（Founder:「我能使用 AI Factory OS 做什么/查看什么/控制什么」）: 新增 `docs/能力地图-做什么-查看什么-控制什么.md` —— **全实测**（命令名单取自 `_top_command_names()` ✓）: 能做（create/chain/run/understand/org member/provider/product/backup）· 能查看（status/dashboard/project show·docs/tasktree 全族/kanban/execution/event/metrics/serve）· 能控制（approval 门/会话三档/tasktree edit·expand·confirm·priority·staffing·workflow/agent assign/llm·provider/workspace init）; **第五节如实列 10 条做不到** ✗; 挂进 AGENTS.md + 守卫（三段齐 + 必须列做不到 ✗ 防吹）| 守卫 75→76 |
