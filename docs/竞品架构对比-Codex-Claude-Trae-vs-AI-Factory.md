...
| MCP | ✅ | ✅ | ❌ 私有 registry | 生态差 |
| 模型 | GPT-5.2/Claude | 顶级+自研 | DeepSeek(可切换, 未接路由) | 变量, 可适配 |
...
### 约束 1: 模型是变量, 不是固定天花板 (v1.1.224 修正)
- Codex/Claude 的"懂人/会干活" = 顶级模型的语义/推理/指令遵循, 但 **harness 可缓解弱模型**
- 实测修正: DeepSeek 网页版纯对话并不差; 我们的"差"一半是 harness 负担(21 工具全量/超长 prompt), 可改
- 正确姿势: 模型无关架构 — Provider 适配 + 每模型 prompt 模板 + 能力协商降级 (见会话诊断 P0-1)
- 已接 LLMRouter/ModelCatalog 基础, 会话链路硬编码 OpenAI 形状属架构债, 待接线
...
- **模型: 可切换, 需接线** — 会话链路接 LLMRouter/ModelCatalog + 适配层即可换; 换强模型收益更大(机制已就位)
