...
def lifecycle_gate(root: Path | str, project_id: str, kind: str, *,
    ...
    - plan: 走 execute_plan (此处不适用)
    返回 {"allowed", "action": CREATE|REFINE|ADVANCE, "target_kind",
          "target_id", "reason"} — 不自行执行, 只 enforcement。
    """
    try:
        data = {k: _load(root, k) for k in ("ideas", "discoveries", "requirements",
                                            "prds", "plans")}
        if kind == "idea":
            return {"allowed": True, "action": "CREATE", "target_kind": "idea",
                    "target_id": "", "reason": "idea 1:N 候选想法"}
        if kind == "discovery":
            if not idea_id:
                return {"allowed": False, "action": "ADVANCE", "target_kind": "idea",
                        "target_id": "", "reason": "discovery 需要上游 idea_id (先建 Idea 链头)"}
            same = [r for r in data["discoveries"].values()
                    if r.get("idea_id") == idea_id
                    and r.get("status") in ("pending", "running")]
            if same:
                t = max(same, key=lambda r: str(r.get("created_at") or ""))
                return {"allowed": False, "action": "REFINE", "target_kind": "discovery",
                        "target_id": str(t.get("id") or ""),
                        "reason": f"该 idea 已有进行中 discovery {t.get('id')} — 深化而非新建"}
            return {"allowed": True, "action": "CREATE", "target_kind": "discovery",
                    "target_id": "", "reason": "discovery 1:N (新分支)"}
        if kind == "requirement":
            ...
            if discovery_id:
                same = [r for r in data["requirements"].values()
                        if r.get("discovery_id") == discovery_id]
                if same:
                    t = max(same, key=lambda r: str(r.get("created_at") or ""))
                    return {"allowed": False, "action": "REFINE", "target_kind": "requirement",
                            "target_id": str(t.get("id") or ""),
        ...
        if kind == "prd":
            alive = [r for r in data["prds"].values()
                     ...
                     and r.get("status") in ("draft", "approved")]
            if alive:
                t = max(alive, key=lambda r: str(r.get("created_at") or ""))
                return {"allowed": False, "action": "REFINE", "target_kind": "prd",
                        "target_id": str(t.get("id") or ""),
                        "reason": f"项目已有 PRD {t.get('id')} ({t.get('status')}) — 用 "
                                  f"record_id 深化 (版本化正文) 或 supersede; 禁止重复 CREATE"}
            return {"allowed": True, "action": "CREATE", "target_kind": "prd",
                    "target_id": "", "reason": "prd CREATE (项目无现存 PRD)"}
        if kind == "plan":
            return {"allowed": False, "action": "ADVANCE", "target_kind": "plan",
                    "target_id": "", "reason": "plan 走 execute_plan (审批→canonical); "
                                               "本工具不直接 CREATE plan"}
        return {"allowed": False, "action": "READ", "target_kind": "",
                "target_id": "", "reason": f"未知 kind {kind}"}
    except Exception as exc:  # noqa: BLE001 — gate 异常 → 保守拒绝 (防失控)
        return {"allowed": False, "action": "READ", "target_kind": "",
                "target_id": "", "reason": f"gate error: {type(exc).__name__}: {exc}"}
