 ...
 * 覆盖:
 * - AfTodoTree 归档: 默认已完成任务在树内显示 (Founder 2026-08-27);
 *   [隐藏已完成] 开关开启 → done 归入归档区; 点击归档任务 → onSelectTask (审计入口)
...
describe('AfTodoTree 归档 (W-3)', () => {
  ...
  it('已完成任务默认在主树; 开启 [隐藏已完成] → 归入归档区', async () => {
    const user = userEvent.setup();
    const { meta } = renderTree();
    render(<AfTodoTree tree={toTodoTree(sampleTodoBacklog(), '演示项目')} taskMeta={meta} />);
    // done 任务 (t-reg-db 用户数据模型) 默认在主树 (Founder 2026-08-27: 完成的任务要显示)
    expect(screen.getByText('用户数据模型')).toBeInTheDocument();
    // 开关默认「隐藏已完成」(未激活), 归档区不渲染
    const toggle = screen.getByTestId('af-tree-hide-completed-toggle');
    expect(toggle).toHaveTextContent('隐藏已完成');
    expect(toggle).toHaveAttribute('aria-pressed', 'false');
    expect(screen.queryByTestId('af-tree-archive')).not.toBeInTheDocument();
    // 开启 → done 从主树隐藏 (主树节点消失), 归档区出现
    await user.click(toggle);
    expect(screen.queryByTestId('af-tree-node-t-reg-db')).not.toBeInTheDocument();
    expect(toggle).toHaveTextContent('已归档 (1)');
    expect(screen.getByTestId('af-tree-archive')).toBeInTheDocument();
  });

  it('开启 [隐藏已完成] → 归档区显示 done 任务; 点击 → onSelectTask', async () => {
    const user = userEvent.setup();
    const { meta } = renderTree();
  ...
  it('全部任务已完成 → 默认全部在树内显示; 开启隐藏已完成 → 空态提示', async () => {
    const user = userEvent.setup();
    ...
    // 默认: done 全部在主树 (已完成任务要在列表里)
    expect(screen.getAllByTestId(/^af-tree-node-/).length).toBeGreaterThan(0);
    ...
    expect(screen.getByTestId('af-tree-hide-completed-toggle')).toHaveTextContent('已归档 (6)');
  });
...
describe('AfTodoTreePage 任务更新 (W-3 集成)', () => {
  ...
  it('归档任务点击 → 详情面板显示 (审计溯源入口)', async () => {
    ...
    render(<AfTodoTreePage projectId="demo" projectName="演示项目" />);
    // 开启「隐藏已完成」→ 点归档区 done 任务 t-reg-db
    await user.click(await screen.findByTestId('af-tree-hide-completed-toggle'));
