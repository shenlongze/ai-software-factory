...
_FACTORY_CORE = ROOT / "src" / "legacy" / "factory-core"
...
PLAN: dict[str, dict[str, list[tuple[str, str]]]] = {
    ...
    "X 主流层风险防护": {
        "命脉级风险 (Founder 2026-08-26 盲点扫描: 影响主流程的地基; 编号避开 W WebUI)": [
            ("X-1", "数据资产保护: git 定期推送 + ~/.factory 备份/导出/迁移 (D-1)", "P0"),
            ("X-2", "LLM provider 故障切换 (D-7): 多 provider 自动降级/告警", "P0"),
            ("X-3", "服务守护/自启: 开机自动恢复 + 进程守护 + 心跳", "P0"),
            ("X-4", "API 认证: 本地 token / 登录 / 权限边界", "P1"),
            ("X-5", "数据结构版本化迁移 (D-3): schema 版本 + 迁移脚本", "P1"),
            ("X-6", "核心执行链 CI 稳定验证: mock/可复现真 LLM 测试", "P1"),
            ("X-7", "操作撤销/确认: 删除/推送二次确认 + 回收站", "P1"),
            ("X-8", "监控自举告警: Monitor 自身健康上报/心跳", "P2"),
        ],
    },
    "V 工具缺口补齐": {
        "39 工具矩阵缺口 (Founder 2026-08-26 盘点, 补全同步)": [
            ("V-10", "工具选择规则设计 (docs/tools-selection-rules.md) — U-1 验收基准", "P0"),
            ("V-2", "测试自动生成工具 (从需求/代码自动生成测试)", "P0"),
            ("V-3", "构建打包工具 (源码→安装包/产物自动化)", "P0"),
            ("V-4", "部署自动化工具 (安装/发布/环境)", "P0"),
            ("V-1", "代码审查工具 (PR/改动自动审查)", "P1"),
            ("V-5", "技术选型评估工具 (同任务换技术/模型对比)", "P1"),
            ("V-7", "代码重构工具 (安全自动重构)", "P1"),
            ("V-8", "回滚工具 (版本回滚/快照恢复)", "P1"),
            ("V-9", "安全评测实测 (8 威胁×防御逐项核对)", "P1"),
            ("V-6", "原型/界面设计工具 (UI 原型生成)", "P2"),
        ],
    },
    "U 工具矩阵统一 (P0)": {
    ...
    "T 会话任务连续 (P0)": {
        "跨会话任务上下文 (Founder 2026-08-26: 会话任务连续)": [
            ...
            ("T-2", "任务上下文注入: 锚定任务后 prompt 带 状态/历史/下一步/exec绑定", "P0"),
            ("T-3", "跨会话恢复: 新会话『继续做 XX』→ 找到上次会话+任务 → 展示『上次进展到哪 + 下一步』", "P0"),
            ("T-4", "任务↔会话双向追溯: 任务能看到哪些会话讨论过它; 会话能看到关联任务", "P0"),
            ("T-5", "端到端实测: 会话A建任务 → 会话B继续 → 完成 → 会话C审计, 全程上下文连贯", "P0"),
        ],
        "任务层连续性 (Founder 2026-08-26: 任务连续也必须要有)": [
            ("T-6", "任务中断恢复验证 (D-2): 执行中断 checkpoint 恢复实测", "P0"),
            ("T-7", "双轨对齐: 版本/战役成果回填任务树 (完成勾选), 计划与执行一致", "P0"),
            ("T-8", "执行链续跑: factory task run 中断后续跑 + exec_state resume 状态回写任务树", "P0"),
            ("T-9", "任务↔执行绑定完整性: 每个任务 exec_ref 可追溯到执行记录/证据包", "P0"),
        ],
    },
    "S 会话×软件打通": {
        "断点修复 (Founder 2026-08-26 盘点)": [
            ("S-1", "会话任务操作: 开始/完成/改优先级/归档 (意图接线 PATCH, 后端已就绪)"),
            ("S-2", "会话建想法模块: '记录个想法 XX' → 建 idea feature"),
            ("S-3", "会话产出物/版本链查询 (产出物 tab 同源)"),
            ("S-4", "会话监控/告警详情 (运维页同源)"),
            ("S-5", "会话设置管理: Agent/Skill/LLM Provider 查询与操作"),
            ("S-6", "会话项目操作: 改名/删除/收藏 (manage 页同源)"),
        ],
    },
}
