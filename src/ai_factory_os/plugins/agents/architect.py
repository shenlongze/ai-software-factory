...
_ARCH_AGENT_PROMPT = (
    ...
    "api_contract, ui_guidance, depends_on, required_capabilities, children}} — 模块/技术任务/"
    "API 约定/UI 实现指导/先决模块/必需能力/子任务, "
    "供 Developer 直接消费)\n"
    "  ★ children = 该模块的**子任务数组**（同结构: 每项含 module/task/api_contract, "
    "还可再嵌套 children ⇒ 任务→子任务→子子任务）。\n"
    "  ★ 【什么时候必须给 children】—— 判据: 若该 module 的任务含 **2 个以上可独立验收的"
    "交付物** ⇒ 必须拆成 children。自检法: 能不能用**一句话**写出它的验收标准? "
    "不能 ⇒ 说明它其实是多件事 ⇒ 拆开。\n"
    "    正例: module='payment', task='接入微信支付 v3: 统一下单、JSAPI 调起、回调验签与"
    "幂等、退款' ⇒ children=[{{task:'统一下单接口'}},{{task:'JSAPI 调起参数与签名'}},"
    "{{task:'回调验签与幂等更新'}},{{task:'退款与部分退款'}}]。\n"
    "    反例: module='infra-scaffold', task='初始化工程脚手架+统一配置' ⇒ **不拆**"
    "（虽长但是一个连续动作, 拆开无意义）。\n"
    "    深度: 一般 1-2 层足够; 只有子任务本身仍含多件可独立验收的事时才再往下拆。\n"
