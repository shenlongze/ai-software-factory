...
_ARCH_PROMPT = (
    ...
    "子任务数组, 可再嵌套 ⇒ 任务→子任务→子子任务)。\n"
    "★ task_breakdown 的拆分粒度【必须拆的具体判据】(否则一个 module 会让一个 agent "
    "一次干太多事, 质量必然下降):\n"
    "  · 若一个 module 的任务含【2 个以上可独立验收的交付物】⇒ 必须给 children 拆开;\n"
    "    自检法: 能不能用【一句话】写出它的验收标准? 不能 ⇒ 说明它其实是多件事 ⇒ 拆。\n"
