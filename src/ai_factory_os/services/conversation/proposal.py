"""conversation · 语义操作层（Semantic Proposal Domain）—— 搬迁自 factory_console.semantic_proposal。

这道闸是整条链的核心设计:
    LLM 只负责**理解语言**（产出 proposal）;**Domain 负责决定 Truth 如何变化**。
    ★ LLM **不得**直接写 Product Understanding —— 唯一写路径必过
      `validate_proposal` → `apply_operations`。
    禁止把 regex keyword parser 扩张成更大的 keyword parser（"生产理解必须走 LLM"）。

10 种语义操作（Capability A-G, §9）:
    ADD / UPDATE / NEGATE / DEFER / REPLACE / CONFIRM / REJECT / CLARIFY / QUESTION / SUGGEST

本模块**零 LLM 依赖**（纯标准库）—— LLM 适配器在同目录 `interpreter.py`。

搬迁说明: 仅把 `from . import product_understanding as pu` 改为 `from . import understanding as pu`
（事实层已迁至同目录 understanding.py），逻辑逐行未动。
"""
