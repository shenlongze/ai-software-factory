"""src/ai_factory_os/services/execution/kernel/cli.py — 执行 CLI (`factory-exec` console script + 主 CLI 共享命令)。
...
def cmd_exec_run(root: Path, args: Any) -> dict:
    ...
    with _logger_scope(root) as logger:
        ...
        # 学习钩子 (执行收尾, 失败安全): 经验入库 —— 与会话入口对齐。
        # 审计 2026-09-14: 本条路径(factory run)此前未接学习, 导致 CLI 跑任务零学习产出。
        # ★ 2026-09-15 已切新地基: services/learning 的 ExperienceAnalyzer.record_experience
        #   （Feedback Loop 入口; 老区 memory/learning_loop 整体要删, 不兼容）
        try:
            analyzer = _open_experience_analyzer(root, logger)
            if analyzer is not None:
                _ok = bool(result.is_success)
                analyzer.record_experience(
                    subject_id=str(getattr(args, "agent", "") or "developer"),
                    subject_type="agent",
                    task_type="development",   # 执行链目前只有开发类任务
                    capability=[],
