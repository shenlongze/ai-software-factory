...
_VERDICT_CONTRACT = "\n".join((
    ...
    "  没有这行 ⇒ 调度器按【没表态】处理（保守：按有没有产出证据复核，不直接算完成）",
    "",
    "★ 收尾纪律（工作区必须干净）:",
    "  · 有产出的任务: 收工前【自己提交】(git add -A && git commit -m \"…\")",
    "    —— 不提交会留下未提交文件, 系统会把这叶标成【待核】(下一个人/下一个叶会踩到这份脏工作区)",
    "  · 确实不该提交的(如纯核验): 在裁定 reason 里写明「不提交, 原因: …」",
))
