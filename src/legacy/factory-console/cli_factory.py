...
def _dep_problems(root: Path) -> list[str]:
    ...
    frontend = root / "@@ALREADY@@" / "src" / "legacy" / "factory-console" / "web" / "frontend"
...
class FactoryCLI:
    ...
    def _start_frontend(self, port: int, *, dev: bool = False) -> bool:
        ...
        frontend = self.root / "@@ALREADY@@" / "src" / "legacy" / "factory-console" / "web" / "frontend"
        if not frontend.is_dir():
    ...
    def _proxy_exec_cli(self) -> Any:
        ...
        try:
            path = str(self.root / "@@ALREADY@@" / "src" / "legacy" / "factory-exec")
    ...
    def _proxy_org_cli(self) -> Any:
        ...
        try:
            path = str(self.root / "@@ALREADY@@" / "src" / "legacy" / "factory-org")
            if path not in sys.path:
                sys.path.insert(0, path)
