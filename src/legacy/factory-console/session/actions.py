"""src/legacy/factory-console/session/actions.py — 真实 Action 集 (S10-048 P0 + S10-049 P0/P2/P3): 调 Service Layer, 不复制业务。
...
# R1 (v1.1.254): 审计动作已拆至 actions_audit (re-export 保兼容)
from .actions_audit import (  # noqa: E402,F401
    audit_events,
    audit_trace,
    audit_chain,
    audit_decision,
...
from .actions_memory import (  # noqa: E402,F401
    memory_search,
    memory_learn,
    memory_stats,
    memory_analyze_agent,
    memory_export,
)
from .actions_debug import (  # noqa: E402,F401
    debug_analyze,
    debug_history,
    debug_recommend,
    debug_stats,
...
def _load_org_cli() -> Any:
    ...
    path = str(root / "ai_factory_os.services.organization" / "src" / "legacy" / "factory-org")
    if path not in sys.path:
        sys.path.insert(0, path)
    return importlib.import_module("ai_factory_os.services.organization.cli")
...
def _load_exec_cli() -> Any:
    ...
    """
    root = REPO_ROOT  # session/ → factory-console/ → 仓库根
    path = str(root / "ai_factory_os.services.organization" / "src" / "legacy" / "factory-exec")
    if path not in sys.path:
        sys.path.insert(0, path)
