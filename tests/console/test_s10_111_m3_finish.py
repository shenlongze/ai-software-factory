...
class TestGlobalVersionDocs:
    ...
    def test_pyproject_version_1_1_78(self):
        ...
        assert re.search(r'^version\s*=\s*"1\.1\.197"', pyproject, re.M), "pyproject 版本非 1.1.197"

    def test_changelog_has_v1_1_78_entry(self):
        changelog = (REPO / "CHANGELOG.md").read_text(encoding="utf-8")
        assert "## [v1.1.197]" in changelog
    ...
    def test_features_md_header_and_m3_rows(self):
        ...
        assert "v1.1.197" in header, f"FEATURES 头版本未更新: {header}"
        # 版本对照表含 v1.1.79 行
        assert "| v1.1.197 |" in features
        # M3-5/6/7 已知缺口行 🚧 → ✅
        row = next(
