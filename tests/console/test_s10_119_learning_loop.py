...
GUARDS = importlib.import_module("factory_console.memory.learning_guards")
LOOP = importlib.import_module("factory_console.memory.learning_loop")
DM = importlib.import_module("factory_console.memory.decision_memory")
EVAL = importlib.import_module("factory_console.session.eval_loop")
...
BUDGET = importlib.import_module("factory_console.session.budget")
LEDGER = importlib.import_module("factory_console.session.cost_ledger")
...
class TestRegistryAndVersion:
    ...
    def test_version_bumped_119(self):
        ...
        assert re.search(r'^version\s*=\s*"1\.1\.206"', pyproject, re.M)
        changelog = (_ROOT / "CHANGELOG.md").read_text(encoding="utf-8")
        assert "## [v1.1.206]" in changelog
        features = (_ROOT / "docs" / "FEATURES.md").read_text(encoding="utf-8")
        assert "v1.1.206" in features
        # K-3/M4-1~6 待办清单同步
