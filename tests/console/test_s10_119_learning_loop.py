...
13. 版本: v1.1.89 断言 (pyproject/CHANGELOG/FEATURES)
...
class TestRegistryAndVersion:
    ...
    def test_version_bumped_119(self):
        """版本 v1.1.89: pyproject + CHANGELOG + FEATURES 同步。"""
        pyproject = (_ROOT / "pyproject.toml").read_text(encoding="utf-8")
        assert re.search(r'^version\s*=\s*"1\.1\.89"', pyproject, re.M)
        changelog = (_ROOT / "CHANGELOG.md").read_text(encoding="utf-8")
        assert "## [v1.1.89]" in changelog
        features = (_ROOT / "docs" / "FEATURES.md").read_text(encoding="utf-8")
        assert "v1.1.89" in features
        # K-3/M4-1~6 待办清单同步
