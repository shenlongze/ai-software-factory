"""tests/dashboard/conftest.py — Dashboard 测试 fixtures (sys.path 兜底同既有模式)。

复用 tests/cli/cli_helpers (run_cli/open_events/event_types) 与 dashboard_helpers
(数据构造), 同 tests/recovery/conftest.py 模式。
"""
...
from dashboard_helpers import (  # noqa: F401
    make_agent,
    make_checkpoint,
    make_execution,
    make_result,
...
def collector(
    ...
    """默认装配的 DashboardCollector (全 store, 只读)。"""
