"""tests/recovery/conftest.py — Checkpoint Recovery 测试 fixtures。

sys.path 兜底同既有模式 (workflows/orchestration); 复用 cli_helpers
(run_cli/open_events/event_types) 与 recovery_helpers (数据构造)。
"""
...
from recovery_helpers import make_agent, make_step, make_task, make_workflow  # noqa: F401
...
@pytest.fixture
def assignments_dir(factory_root: Path) -> Path:
    return factory_root / "assignments"
...
def service(
    ...
    """默认装配的 RecoveryService (全部存储 + 事件记录)。"""
...
def service_no_logger(
    ...
    """无 logger 的纯存储 RecoveryService (事件无关测试)。"""
    return RecoveryService(
        task_store=task_store,
        workflow_store=workflow_store,
        assignment_store=assignment_store,
